pipeline {
  ...
  stages {
    stage('Build') {
      steps {
        bat 'nuget restore'
        changeAsmVer(assemblyFile: 'LoL Auto Login\\Properties\\AssemblyInfo.cs', regexPattern: 'Assembly(\\w*)Version\\("(\\d+).(\\d+).(\\d+).(\\d+)"\\)', replacementPattern: 'Assembly$1Version("$2.$3.$4.%s")', versionPattern: '$BUILD_NUMBER')
